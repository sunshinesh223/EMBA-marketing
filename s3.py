    ...
    for attempt in range(max_retries):
        try:
            ...
            if "message" in response_json and "content" in response_json["message"]:
                ...
                print(f"API Response Content (Attempt {attempt + 1}):\n{content}\n")

                # Use regex to extract relevance scores from the content
                scores = re.findall(r"Relevance score:\s*([\d.]+)", content)

                # If parsing fails or results are incomplete, retry the API call
                if len(scores) != len(prompts):
                    print(f"Warning: Expected {len(prompts)} scores, but got {len(scores)}. Retrying (Attempt {attempt + 1})...")
                    continue  # Retry by making another API call

                return [float(score) for score in scores]

            else:
                print("Unexpected response structure:", response_json)
                continue  # Retry if the structure is unexpected

        except requests.exceptions.RequestException as e:
            print(f"Request failed on attempt {attempt + 1}: {e}")
            continue  # Retry the request in case of a failure

    # If all retries fail, return a list of default scores
    print(f"Failed after {max_retries} attempts. Returning default scores.")
...
def process_attribute(attribute, reviews_df, batch_size=5):
    ...
    with tqdm(total=total_batches, desc=f"Processing {attribute.capitalize()}") as pbar:
        for i in range(0, len(reviews_df), batch_size):
            ...
            batch_scores = analyze_sentiment_if_relevant_batch(reviews_batch, attribute)
