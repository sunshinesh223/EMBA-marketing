...
def llama_batch(prompts, max_retries=2):
    ...
    # Retry mechanism in case of request failures
        ...
        try:
            ...
            if "message" in response_json and "content" in response_json["message"]:
                ...
                if len(scores) != len(prompts):
                    print(
                        f"Warning: Expected {len(prompts)} scores, but received {len(scores)}. Filling missing values with None.")

                    if attempt < max_retries - 1:
                        print(f"Retrying due to mismatched from llm")
                        continue
                    else:
                        # Fill missing scores with None to match the expected length
    ...
    # If all retries fail or response is invalid, return a list of None scores
...
desired_reviews_per_company = 600 // reviews_df['source'].nunique()
...
reviews_df_balanced = reviews_df.groupby('source', group_keys=False).apply(
    lambda x: x.sample(min(len(x), desired_reviews_per_company))
)
# If the total sample size is less than 500 due to companies with fewer reviews, you can adjust accordingly:
reviews_df_balanced = reviews_df_balanced.sample(n=min(len(reviews_df_balanced), 500)).reset_index(drop=True)
...
attributes = ['cabin experience', 'price comparison', 'service quality','Dining experience','Entertainment experience',]
...
def determine_relevance_batch(reviews, attribute, batch_size=1):
    ...
    for i in range(0, len(reviews), batch_size):
        ...
        prompt = f"""
        I need you first forget everything in the context, start refresh. Take this message as the first and the last message. 
...
def process_attribute(attribute, reviews_df, batch_size=1):
    if attribute + '_sentiment' not in reviews_df.columns:
        # Initialize the column with NaN if it does not exist
        reviews_df[attribute + '_sentiment'] = np.nan
    ...
    with tqdm(total=total_batches, desc=f"Processing {attribute.capitalize()}") as pbar:
        for i in range(0, len(reviews_df), batch_size):
            # Select the batch of reviews to process
            reviews_batch_df = reviews_df.iloc[i:i + batch_size]

            # Filter out reviews that already have a sentiment score (i.e., not NaN)
            reviews_to_process = reviews_batch_df[reviews_batch_df[attribute + '_sentiment'].isna()]

            # If there are reviews left to process in this batch, send them to the LLM
            if not reviews_to_process.empty:
                batch_scores = analyze_sentiment_if_relevant_batch(reviews_to_process, attribute)

                # Ensure the length of batch_scores matches the number of rows being updated
                if len(batch_scores) != len(reviews_to_process):
                    print(f"Warning: Mismatch in length at batch {i // batch_size}. Expected {len(reviews_to_process)}, got {len(batch_scores)}.")
                    # Adjust the indices to match the length of batch_scores
                    reviews_df.loc[reviews_to_process.index[:len(batch_scores)], attribute + '_sentiment'] = batch_scores
                else:
                    reviews_df.loc[reviews_to_process.index, attribute + '_sentiment'] = batch_scores
